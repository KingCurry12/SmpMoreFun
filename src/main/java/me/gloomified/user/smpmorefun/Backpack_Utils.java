...
public class Backpack_Utils {
    public static void storeItems(List<ItemStack> items, Player p){
        PersistentDataContainer data = p.getPersistentDataContainer();
        if (items.size() == 0){
            data.set(new NamespacedKey(SmpMoreFun.getPlugin(SmpMoreFun.class), "Backpack"), PersistentDataType.STRING, "");
        }else{
            try{
                ...
                data.set(new NamespacedKey(SmpMoreFun.getPlugin(SmpMoreFun.class), "Backpack"), PersistentDataType.STRING, encodedData);
                os.close();
    ...
    public static ArrayList<ItemStack> getItems(Player p){
        ...
        String encodedItems = data.get(new NamespacedKey(SmpMoreFun.getPlugin(SmpMoreFun.class), "Backpack"), PersistentDataType.STRING);
